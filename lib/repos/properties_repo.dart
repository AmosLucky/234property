...
class PropertiesRepo {
  ...
  Future<String> fetchPropertyDetails(id) async {
    String url =
  ...
  Future searchProperties(String search) async {
    var searchUrl =
        "https://234property.chosencrusader.com/admin/process/list?pageType=m-properties&mobile=1&user_id=1&condition=status,1&combine=and&search="+search;
    try {
      http.Response response = await http.get(
        Uri.parse(searchUrl),
      );
